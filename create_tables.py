...
def create_database():
    ...
    conn = psycopg2.connect("host=127.0.0.1 dbname=mydatabase user=postgres password=postgres")
    conn.set_session(autocommit=True)
    cur = conn.cursor()
    
    # create sparkify database with UTF8 encoding
    cur.execute("DROP DATABASE IF EXISTS travauxdb")
    cur.execute("CREATE DATABASE travauxdb WITH ENCODING 'utf8' TEMPLATE template0")

    # close connection to default database
    conn.close()    
    
    # connect to travauxdb database
